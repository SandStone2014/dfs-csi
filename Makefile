...
juicefs-csi-driver:
	mkdir -p bin
	CGO_ENABLED=0 GOOS=linux go build -gcflags "all=-N -l" -o bin/juicefs-csi-driver ./cmd/driver

.PHONY: warmup
warmup:
	mkdir -p bin
	CGO_ENABLED=0 GOOS=linux go build -gcflags "all=-N -l" -o bin/warmup ./cmd/warmup
...
.PHONY: deploy
deploy: deploy/kubernetes/v1.17-v1.21/deploy/csi-plugin/k8s.yaml
	kubectl apply -f $<

.PHONY: deploy-delete
uninstall: deploy/kubernetes/v1.17-v1.21/deploy/csi-plugin/k8s.yaml
	kubectl delete -f $<
...
.PHONY: clean
clean:
	rm -rf build_package && rm -rf bin

.PHONY: mosfs-csi-image
mosfs-csi-image:
	docker build -t sandstone/mosfs-csi-driver:v1.1.6  .

.PHONY: warmup-image
warmup-image:
	docker build -t sandstone/cache-controller:v1.1.6 -f warmup.Dockerfile .

.PHONY: only-build-package
only-build-package:
	sh hack/build-package.sh v1.1.6

.PHONY: package
package: clean mosfs-csi-image warmup-image
	bash hack/build-package.sh v1.1.6
