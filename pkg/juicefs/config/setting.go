...
func ParseSetting(secrets, volCtx map[string]string, usePod bool) (*JfsSetting, error) {
	...
	if volCtx != nil {
		jfsSetting.MountPodCpuLimit = volCtx["mountpod/cpu-limit"]
		jfsSetting.MountPodMemLimit = volCtx["mountpod/memory-limit"]
		jfsSetting.MountPodCpuRequest = volCtx["mountpod/cpu-request"]
		jfsSetting.MountPodMemRequest = volCtx["mountpod/memory-request"]
		delay := volCtx["mountpod/mount-delete-delay"]
		if delay != "" {
